...
이러한 헤더가 어떻게 작동하는지 살펴보겠습니다. 이 예제에서는 [!DNL Experience Cloud] ID 서비스를 사이트 `www.finance-website.com`에 구현한 금융 서비스 회사가 있다고 가정합니다. 다음 테이블은 CORS 요청 및 응답 헤더가 리소스에 대한 액세스를 확인하는 방법을 정의합니다.
...
아래 테이블은 CORS가 ID 서비스를 사용하는 고객에게 제공하는 몇 가지 이점을 설명합니다.
