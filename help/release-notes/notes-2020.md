...
* `ESLint`에서 보고한 문제를 해결하기 위해 `Internet-Explorer (IE)` 버전을 감지하는 데 사용되는 함수를 업데이트했습니다.
ECID에 optIn `pre-approval`이(가) 부여되고 나중에 업데이트되는 경우 `Internet-Explorer (IE) 11`의 성능 문제를 수정했습니다.
