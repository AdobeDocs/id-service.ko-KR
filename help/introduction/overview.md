...
>[!TIP]
>
>방문자 ID 서비스, Experience Platform Identity 서비스 및 ECID는 세 개의 **서로 다른** 엔터티입니다.

방문자 ID 서비스는 다른 애플리케이션별 ID를 대체하고 [고객 ID 및 인증 상태](/help/reference/authenticated-state.md) 기능을 사용하여 고객 ID를 CX Enterprise에 전달할 수 있도록 해 줍니다.

>[!NOTE]
>
>방문자 ID 서비스 는 현재 구독 중인 CX 엔터프라이즈 애플리케이션 서비스에서만 작동하며, 구독하지 않는 경우 다른 애플리케이션 서비스에 대한 액세스를 제공하지 않습니다.
...
요약하면 방문자 ID 서비스는 다음과 같은 작업을 지원합니다.

* 여러 애플리케이션에 걸쳐 디바이스에서 방문자를 고유하게 식별합니다.
* 동일한 도메인에서 추적할 수 있도록 고객 도메인에 자사 쿠키를 설정합니다. 자세한 내용은 [쿠키 및 방문자 ID 서비스](./cookies.md)에 대한 문서를 참조하십시오.
...
방문자 ID 서비스를 사용하려면 솔루션 및 기타 Adobe 코드 라이브러리가 [특정 요구 사항](/help/reference/requirements.md)을 충족해야 합니다.

* [쿠키 및 방문자 ID 서비스](cookies.md): 방문자 ID 서비스는 IMS 조직 ID, CX Enterprise AMCV 쿠키 및 demdex 쿠키를 사용하여 사이트 방문자에 대한 고유하고 영구적인 식별자를 만들어 저장합니다. 이러한 쿠키를 사용하면 방문자 ID 서비스가 다른 도메인의 방문자를 추적하고 다른 CX 엔터프라이즈 솔루션 간에 데이터 공유를 사용할 수 있습니다.
* [방문자 ID 서비스에서 ID를 요청하고 설정하는 방법](id-request.md): ID 요청 및 응답 프로세스에 대한 개요입니다. 이러한 예제에서는 개별 사이트, 여러 다른 사이트 및 자체 IMS 조직 ID가 있는 다른 CX Enterprise 고객이 관리하는 사이트에 대한 ID 할당을 다룹니다.
* [ID 동기화 및 일치율 이해](match-rates.md): Adobe Media Optimizer 및 Visitor ID 서비스를 비롯한 방문자 ID 서비스의 ID 동기화 프로세스 및 일치율에 대한 개요입니다.
