---
description: 이러한 지침은 방문자 ID 서비스를 사용하고 태그는 사용하지 않으려는 Analytics 및 Audience Manager 고객을 대상으로 합니다. 그러나 태그를 사용하여 방문자 ID 서비스를 구현하는 것이 좋습니다. 태그를 사용하면 구현 워크플로를 간소화할 수 있으며 올바른 코드 배치 및 순서를 자동으로 확인할 수 있습니다.
keywords: 방문자 ID 서비스
title: Analytics 및 Audience Manager용 Adobe 방문자 ID 서비스 구현
...
# Analytics 및 Audience Manager용 Adobe 방문자 ID 서비스 구현{#implement-the-experience-cloud-id-service-for-analytics-and-audience-manager}

이러한 지침은 방문자 ID 서비스를 사용하고 [태그](https://experienceleague.adobe.com/docs/experience-platform/tags/home.html?lang=ko-KR)는 사용하지 않으려는 Analytics 및 Audience Manager 고객을 대상으로 합니다. 그러나 태그를 사용하여 방문자 ID 서비스를 구현하는 것이 좋습니다. 태그를 사용하면 구현 워크플로를 간소화할 수 있으며 올바른 코드 배치 및 순서를 자동으로 확인할 수 있습니다.
...
1. **[!UICONTROL Admin]** > **[!UICONTROL Code Manager]**(으)로 이동합니다.
...
## 9단계: 방문자 ID 서비스 코드 테스트 및 배포 {#section-f857542bfc70496dbb9f318d6b3ae110}

다음과 같이 테스트하고 배포할 수 있습니다.

**테스트 및 확인**

방문자 ID 서비스 구현을 테스트하려면 다음을 확인하십시오.
...
* [방문자 ID 서비스 테스트 및 확인](../implementation-guides/test-verify.md)도 참조하세요.
